...
public interface ParkingLotRepository extends JpaRepository<ParkingLot, Long> {
    ...
    @Query("SELECT p FROM ParkingLot p WHERE p.active = true AND " +
            ...
            "(:search IS NULL OR :search = '' OR LOWER(p.name) LIKE LOWER(CONCAT('%', CAST(:search AS string), '%')) OR LOWER(p.address) LIKE LOWER(CONCAT('%', CAST(:search AS string), '%')))")
    List<ParkingLot> searchParkingLots(@Param("city") String city, @Param("search") String search);
